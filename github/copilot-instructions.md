...
These rules apply to every file generated or modified in the `ai-architect-ui/` directory.

---

### RULE U-1 — Read the UX style guide before generating any UI code

The style guide lives at `ai-architect-ui/UX-STYLE-GUIDE.md`. It is the
authoritative reference for all visual and interaction decisions. Read it
before generating any new component, view, or UI change.

If a pattern already exists in the style guide, use it exactly. Do not
invent new colours, spacing values, border radii, or animation classes.

---

### RULE U-2 — Use design tokens, never raw values

All colours used in Tailwind classes must come from the theme tokens
defined in `src/index.css`:

  bg-accent             ← primary action green (#10a37f)
  bg-accent-hover       ← primary button hover (#0d8c6d)
  bg-sidebar            ← sidebar background (#202123)
  bg-sidebar-hover      ← sidebar hover / active (#2A2B32)
  border-sidebar-border ← sidebar borders (#4E4F60)

Do not write `bg-[#10a37f]`, `text-[#202123]`, or any other inline hex
value. If a colour is needed and not in the token set, add it to
`@theme {}` in `index.css` before using it.

---

### RULE U-3 — Icon conventions

All icons are inline SVG. Do not import external icon libraries.
...
strokeWidth conventions:
  1.5  — outline / structural icons
  2    — action icons, bolder UI icons
  2.5  — status icons (check, cross, dash)

Sizes:
  w-3.5 h-3.5  — sub-label / inline status
  w-4 h-4      — standard UI icon (most common)
  w-6 h-6      — large / section hero
  w-7 h-7      — branding icon inside logo container

Do not use emoji, Unicode symbols, or border-based spinners.

---

### RULE U-4 — Buttons must always be accessible

Every `<button>` must have:
  - Visible label text OR `aria-label`
  - `transition-colors`
  - `disabled:opacity-40` if the button can be disabled
  - `focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2`
    (primary) or equivalent visible focus ring for each variant

Never use `cursor-pointer` on a `<button>` (it is the default). Use
`cursor-not-allowed` on explicitly disabled interactive elements.

See the "Buttons" section of UX-STYLE-GUIDE.md for the exact class
strings for each button variant.

---

### RULE U-5 — No motion outside the approved set

The only acceptable entrance animation is the `.archon-reveal` CSS class
defined in `src/index.css`. Apply it only to view-level static sections
(HomeView staggered sections). Do not apply it to dynamically inserted
content, chat messages, error banners, or loading states.

Use `transition-colors` on all hover-interactive elements.
Do not add `transition-all`, Framer Motion, or any other animation
library without explicit approval.

The `@media (prefers-reduced-motion: reduce)` rule in `index.css`
disables `.archon-reveal` automatically. Do not override this.

---

### RULE U-6 — Every interactive and structural element needs data-testid

All view roots, significant interactive elements, loading states, error
states, and form fields must carry a `data-testid` attribute so Vitest
tests can query them without relying on text content.

Naming convention:
  View root:       data-testid="home-view"
  Nav items:       data-testid="nav-{key}"
  Stage rows:      data-testid="stage-{name}"
  States:          data-testid="governance-loading", "governance-error"
  Form fields:     data-testid="auth-email", "auth-password"

The `data-testid` attribute must be the last attribute on the element.

---

### RULE U-7 — Carry the sidebar scroll class only inside the sidebar

The `.sidebar-scroll` CSS class applies custom scrollbar styling.
It must only be used on elements inside the sidebar. All other scrollable
regions use the browser default scrollbar.